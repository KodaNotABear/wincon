// Pull your recent ranked games to disk. Cached matches are never re-fetched,
// so re-running sync only costs API calls for games you haven't seen yet.
//
//   npm run sync
...
import { DATA_DIR, MATCH_DIR, loadEnv, requireEnv } from './env'

loadEnv()

const apiKey = requireEnv('RIOT_API_KEY', 'Copy .env.example to .env and add your key from developer.riotgames.com')
const riotId = requireEnv('RIOT_ID', 'Set RIOT_ID in .env, e.g. RIOT_ID=YourName#NA1')
...
if (!gameName || !tagLine) {
  console.error(`RIOT_ID must look like YourName#NA1, got "${riotId}"`)
  process.exit(1)
}
...
let cached = 0
for (const id of ids) {
  const file = path.join(MATCH_DIR, `${id}.json`)
...
console.log(`\nDone: ${fetched} new, ${cached} cached. Next: npm run analyze`)
